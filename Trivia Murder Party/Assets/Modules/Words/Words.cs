...
public class Words : MonoBehaviour
{
    ...
    public Material RegularFontMat;
        ...
        for (int i = 0; i < 24; i++)
        {
            if (Button == Buttons[i])
            {
                ...
                else
                {
                    for (int q = 0; q < 3; q++)
                    {
                        for (int j = 0; j < MinorWordList.AllWords[q].Length; j++)
                        { //With the original way I had this, the giant word list was an array, so I wanted to search the smaller list first.
                    ...
                    if (AllWordList.List.Contains(CurrentSubmission))
                    { //I then looked at boggle's code, since it is fast, and it is a simple .Contains()
                        IsAWord = true;
                    }
                    if (IsAWord)
                    {
    ...
    IEnumerator PointsDisplay()
    {
        ...
        {
            SetStandardSquareAttributes(i);
            Squares[i].SquareText.text =  "";
            yield return new WaitForSecondsRealtime(.1f);
        }
        for (int i = 4; i < 9; i++)
        {
            Squares[i].SquareText.text =  "TOTAL"[i - 4].ToString();
            yield return new WaitForSecondsRealtime(.1f);
        }
        for (int i = 9; i < 14; i++)
        {
            Squares[i].SquareText.text =  "POINT"[i - 9].ToString();
            yield return new WaitForSecondsRealtime(.1f);
        }
        for (int i = 14; i < 19; i++)
        {
            Squares[i].SquareText.text =  "EQUAL"[i - 14].ToString();
            yield return new WaitForSecondsRealtime(.1f);
        }
        if (TotalPoints >= 0)
        {
            Squares[20].SquareText.text = "+";
        }
        else
        {
            Squares[20].SquareText.text = "-";
        ...
        yield return new WaitForSecondsRealtime(.1f);
        Squares[21].SquareText.text = (Math.Abs(TotalPoints / 10) % 10).ToString();
        yield return new WaitForSecondsRealtime(.1f);
        Squares[22].SquareText.text = Math.Abs(TotalPoints % 10).ToString();
        yield return new WaitForSecondsRealtime(1f);
        StartCoroutine(WordGeneration());
    }
    ...
    IEnumerator WordGeneration()
    {
        ...
        for (int i = 0; i < 24; i++)
        {
            SetRandomSquareAttributes(i);
            Squares[i].SquareText.text =  ShownLetters[i].ToString();
            yield return new WaitForSecondsRealtime(.1f);
        }
        Animating = false;
    ...
    private void SetRandomSquareAttributes(int i)
    {
        Squares[i].SquareText.text = ShownLetters[i].ToString();
        Squares[i].SquareRenderer.material.color = Color.HSVToRGB(Rnd.Range(0f, 1f), 0.65f, 0.8f);
        int font = Rnd.Range(0, Fonts.Length);
        Squares[i].SquareText.font = Fonts[font];
        Squares[i].SquareText.GetComponent<MeshRenderer>().material = FontMats[font];
        float randRot = Rnd.Range(-10f, 10f);
        Squares[i].transform.localEulerAngles = new Vector3(270, randRot, 0);
        int rndMesh = Rnd.Range(0, Meshes.Length);
        Squares[i].SquareFilter.mesh = Meshes[rndMesh];
    }

    private void SetStandardSquareAttributes(int i)
    {
        Squares[i].SquareRenderer.material.color = new Color32(200, 200, 200, 255);
        Squares[i].SquareText.font = RegularFont;
        Squares[i].SquareText.GetComponent<MeshRenderer>().material = RegularFontMat;
        Squares[i].SquareText.text = "";
        Squares[i].transform.localEulerAngles = new Vector3(270, 0, 0);
        int rndMesh = Rnd.Range(0, Meshes.Length);
        Squares[i].SquareFilter.mesh = Meshes[rndMesh];
    }
    ...
    IEnumerator TwitchHandleForcedSolve()
    {
        ...
        while (!moduleSolved)
        {
            ...
            for (int i = 0; i < ChosenWord.Length; i++)
            {
                for (int j = 0; j < 24; j++)
                {
                    if (Squares[j].SquareText.text == ChosenWord[i].ToString() && !HaveBeenPressed[j] && !HasPressed)
                    {
